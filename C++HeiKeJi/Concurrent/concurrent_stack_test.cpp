...
void BM_stack0_cas(benchmark::State &state) {
  ...
  for (auto _ : state) {
    for (size_t i = 0; i < N; ++i) {
      ...
      n.compare_exchange_weak(nn, nn, std::memory_order_release,
                              std::memory_order_relaxed);
      s0.push(i);
    }
    for (size_t i = 0; i < N; ++i) {
      size_t nn = n.load(std::memory_order_relaxed);
      n.compare_exchange_weak(nn, nn, std::memory_order_acquire,
                              std::memory_order_relaxed);
      benchmark::DoNotOptimize(s0.pop());
    }
  }
...
void BM_stackrw(benchmark::State &state) {
  ...
  for (auto _ : state) {
    for (size_t i = 0; i < N; ++i)
      srw.push(i);
    for (size_t i = 0; i < N; ++i)
      benchmark::DoNotOptimize(srw.pop());
  }
  state.SetItemsProcessed(state.iterations() * N);
}
...
void BM_stackrw1(benchmark::State &state) {
  ...
  for (auto _ : state) {
    for (size_t i = 0; i < N; ++i)
      srw1.push(i);
    for (size_t i = 0; i < N; ++i)
      benchmark::DoNotOptimize(srw1.pop());
  }
  state.SetItemsProcessed(state.iterations() * N);
}
...
void BM_stack0_top(benchmark::State &state) {
  ...
  for (auto _ : state) {
    for (size_t i = 0; i < N; ++i)
      benchmark::DoNotOptimize(s0.top());
  }
  state.SetItemsProcessed(state.iterations() * N);
}
...
void BM_stack1_top(benchmark::State &state) {
  ...
  for (auto _ : state) {
    for (size_t i = 0; i < N; ++i)
      benchmark::DoNotOptimize(s1.top());
  }
  state.SetItemsProcessed(state.iterations() * N);
}
...
void BM_stackrw_top(benchmark::State &state) {
  ...
  for (auto _ : state) {
    for (size_t i = 0; i < N; ++i)
      benchmark::DoNotOptimize(srw.top());
  }
  state.SetItemsProcessed(state.iterations() * N);
}
...
void BM_stackrw1_top(benchmark::State &state) {
  ...
  for (auto _ : state) {
    for (size_t i = 0; i < N; ++i)
      benchmark::DoNotOptimize(srw1.top());
  }
  state.SetItemsProcessed(state.iterations() * N);
}
