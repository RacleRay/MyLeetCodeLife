...
public:
  ...
  void push(const T &v) {
    ...
    if (n.pc_ == cap_)
      abort();
    int i = 0;
    while (!n.equal(n_) || !n_.compare_exchange_weak(
                               n, {n.pc_ + 1, n.cc_}, std::memory_order_acquire,
    ...
    if (!n_.compare_exchange_strong(n, {n.pc_, n.cc_ + 1},
  ...
  std::optional<T> pop() {
    ...
    int i = 0;
    while (!n.equal(n_) || !n_.compare_exchange_weak(
                               n, {n.pc_, n.cc_ - 1}, std::memory_order_acquire,
                               std::memory_order_relaxed)) {
      if (n.cc_ == 0)
        return std::optional<T>(std::nullopt);
      nanosleep(i);
    ...
    if (!n_.compare_exchange_strong(n, {n.pc_ - 1, n.cc_},
                                    std::memory_order_release,
                                    std::memory_order_relaxed))
      abort();
    return res;
