...
public:
  ...
  void lock() {
    while (true) {
      if (flag_.fetch_sub(idle, std::memory_order_acquire) == idle)
        return;                                         // flag_ == 0
  ...
  void unlock() { flag_.fetch_add(idle, std::memory_order_release); }
  void lock_shared() {
    while (true) {
      if (flag_.fetch_sub(1, std::memory_order_acquire) > 0)
        return;                                      // flag_ >= 0
  ...
  void unlock_shared() { flag_.fetch_add(1, std::memory_order_release); }
