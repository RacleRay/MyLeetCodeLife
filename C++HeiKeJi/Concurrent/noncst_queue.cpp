...
public:
  explicit subqueue(size_t capacity)
      : capacity_(capacity), begin_(0), size_(0) {}
...
public:
  explicit concurrent_queue(size_t capacity = 1UL << 15) {
    for (int i = 0; i < QUEUE_COUNT; ++i) {
      queues_[i].queue.store(subqueue_t::construct(capacity),
  ...
  ~concurrent_queue() {
    for (int i = 0; i < QUEUE_COUNT; ++i) {
      subqueue_t *queue =
          queues_[i].queue.exchange(nullptr, std::memory_order_relaxed);
  ...
  std::optional<T> pop() {
    ...
    bool success = false;
    for (size_t i = 0;;) {
      i = dequeue_slot_.fetch_add(1, std::memory_order_relaxed) &
          (QUEUE_COUNT - 1);
      queue = queues_[i].queue.exchange(
          nullptr, std::memory_order_acquire); // Take ownership of the subqueue
      if (queue) {
        success = queue->pop(res); // Dequeue element while we own the queue
        queues_[i].queue.store(
            queue, std::memory_order_release); // Relinquish ownership
        if (success)
          break; // We have an element
        if (count_.load(std::memory_order_acquire) == 0)
          goto EMPTY; // No element, and no more left
      } else {        // queue is NULL, nothing to relinquish
        if (count_.load(std::memory_order_acquire) == 0)
          goto EMPTY; // We failed to get a queue but there are no more entries
                      // left
      }
      if (success)
        break;
  ...
  bool push(const T &v) {
    ...
    for (size_t i = 0;;) {
      ...
      i = enqueue_slot_.fetch_add(1, std::memory_order_relaxed) &
          (QUEUE_COUNT - 1);
      queue = queues_[i].queue.exchange(
          nullptr, std::memory_order_acquire); // Take ownership of the subqueue
      if (queue) {
        ...
        queues_[i].queue.store(
            queue, std::memory_order_release); // Relinquish ownership
        if (success)
          return success; // We added the element
        if (++full_count == QUEUE_COUNT)
          break; // We tried hard enough, probably queue is full
      }
      static const struct timespec ns = {0, 1};
      nanosleep(&ns, NULL);
    };
