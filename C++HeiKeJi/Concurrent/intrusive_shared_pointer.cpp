...
//     bool DelRef() - atomically decrement reference count by 1, return true
//     iff the counter dropped to 0 operator T&(), operator const T&() const -
//     conversions to T (implicit if U is derived from T)
...
class intr_shared_ptr {
  struct get_ptr {
    ...
    get_ptr(std::atomic<U *> &ptr) : aptr(ptr), p() {
      ...
      for (int i = 0;
           aptr.load(std::memory_order_relaxed) == (U *)(locked) ||
           (p = aptr.exchange((U *)(locked), std::memory_order_acquire)) ==
               (U *)(locked);
           ++i) {
        if (i == 8) {
          i = 0;
    ...
    ~get_ptr() { aptr.store(p, std::memory_order_release); }
...
public:
  ...
  public:
    ...
    bool operator==(const shared_ptr &rhs) const { return p_ == rhs.p_; }
    bool operator!=(const shared_ptr &rhs) const { return p_ != rhs.p_; }
  ...
  explicit operator bool() const {
    return p_.load(std::memory_order_relaxed) != NULL;
  }
  ...
  bool compare_exchange_strong(shared_ptr &expected_ptr,
                               const shared_ptr &new_ptr) {
    get_ptr p(p_);
...
struct A {
  ...
  volatile A &operator=(const A &rhs) volatile {
    i = rhs.i;
    return *this;
  }
};
...
struct B : public A {
  B(int i = 0) : A(i), ref_cnt_(0) { ++B_count; }
  ~B() { --B_count; }
  B(const B &x) = delete;
...
void BM_ptr_xassign(benchmark::State &state) {
  if (state.thread_index() == 0)
    p = intr_shared_ptr<A, B>(new B(42)), q = intr_shared_ptr<A, B>(new B(7));
